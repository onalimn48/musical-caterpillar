...
export default function Staff({ note, clef, highlight, showLabel, extended, dark, shake, songMode }) {
  const pos = getNotePosition(note, clef, { extended, songMode });
  const noteLetter = note.charAt(0);
  const noteY = getPositionY(pos);
  const noteX = 65;
  const ledgers = getLedgerLines(pos);
  const extraBottom = Math.max(0, Math.ceil(noteY + (showLabel ? 22 : 10) - (STAFF_H - 8)));
  const svgHeight = STAFF_H + extraBottom;
  ...
  const stemUp = pos < 4;
  const clefTextY = clef === "alto"
    ? TOP_LINE_Y + 4.04 * LINE_GAP
    : TOP_LINE_Y + 3.55 * LINE_GAP;
  const clefFontSize = clef === "alto" ? 86 : 80;
  return (
    <svg width={STAFF_W} height={svgHeight} viewBox={`0 0 ${STAFF_W} ${svgHeight}`}
      ...
      {clef === "treble" ? (
        ...
        <text x="22" y={clefTextY} fontSize={clefFontSize} fill={clefColor}
          fontFamily="'Noto Music','Segoe UI Symbol',serif" textAnchor="middle">{getClefSymbol(clef)}</text>
      )}
      {ledgers.map((lp, i) => (
        <line key={`l${i}`} x1={noteX - 14} y1={getPositionY(lp)} x2={noteX + 14} y2={getPositionY(lp)}
          stroke={lineColor} strokeWidth="1" />
      ))}
      ...
      {showLabel && (
        <text x={noteX} y={svgHeight - 8} textAnchor="middle" fontSize="15" fontWeight="700"
