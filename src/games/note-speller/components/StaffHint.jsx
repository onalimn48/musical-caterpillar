    ...
    <div style={{
      ...
      <div style={{
        ...
        <svg
          ...
          height="160"
          viewBox="0 0 320 160"
          style={{ width: "100%", maxWidth: 320, height: "auto", display: "block", marginBottom: 12 }}
        >
          {[0, 1, 2, 3, 4].map(i => (
            <line key={i} x1="10" y1={topLineY + i * lineGap} x2="310" y2={topLineY + i * lineGap} stroke="#8d7e72" strokeWidth="1.2" />
          ))}

          {clef === "treble" ? (
            <g transform={`translate(10, ${topLineY - 1.08 * lineGap}) scale(${lineGap * 6.65 / 4066})`}>
              <path
            ...
            <>
              <text
                x={clef === "alto" ? "22" : "32"}
                y={topLineY + (clef === "alto" ? 4.03 : 3.3) * lineGap}
          ...
          {hint.lines.map((note, i) => {
            ...
            return (
              <g key={`line-${note}-${i}`}>
                <circle cx={60 + i * 55} cy={topLineY + (4 - i) * lineGap} rx="8" ry="6" fill={color} />
                <text x={60 + i * 55} y={topLineY + (4 - i) * lineGap + 4.5} textAnchor="middle" fontSize="11" fontWeight="700" fill="white" fontFamily="'Fredoka',sans-serif">{note}</text>
              </g>
            );
          })}
          ...
          {hint.spaces.map((note, i) => {
            ...
            return (
              <g key={`space-${note}-${i}`}>
                <circle cx={80 + i * 55} cy={topLineY + (3.5 - i) * lineGap} rx="8" ry="6" fill={color} opacity="0.85" />
                <text x={80 + i * 55} y={topLineY + (3.5 - i) * lineGap + 4.5} textAnchor="middle" fontSize="11" fontWeight="700" fill="white" fontFamily="'Fredoka',sans-serif">{note}</text>
              </g>
            );
          })}
        </svg>
